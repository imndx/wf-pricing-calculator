...
const computedCommerialProductList = computed(() => {
    let map = new Map()
...
<template>
    <div class="greetings">
        ...
        <div v-else-if="curReqIndex === computedProductRequirementList.length" style="height: 100%">
            <div v-if="checkedProductRequirementList.filter(req => req.id !== 'linux').length > 0" class="checked_req_item">
                <div style="flex: 1">
                    <p class="title">你的产品需求如下，请确认</p>
                    <div v-for="(req, index) in checkedProductRequirementList.filter(req => req.id !== 'linux')" :key="index">
                        <p class="desc">{{ req.name }}</p>
                    </div>
                </div>
                <div class="action-container">
                    <button @click="initProductRequirementList">
        ...
        <div v-else class="report">
            <div style="flex: 1">
                ...
                <p>{{ '总价：' + computedCommerialProductList.map(p => p.price).reduce((pre, cur, index) => pre + cur, 0) }}</p>
                <p>费用详情：</p>
                <div v-for="(cp, index) in computedCommerialProductList" :key="index">
